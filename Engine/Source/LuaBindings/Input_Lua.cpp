...
int Input_Lua::ShowSoftKeyboard(lua_State* L)
{
    ...
    const char* text = INP_ShowSoftKeyboard(show);

    if (show && text != nullptr)
    {
...
void Input_Lua::BindKeyTable()
{
    ...
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_BACK);
    lua_setfield(L, tableIdx, "Back");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_ESCAPE);
    lua_setfield(L, tableIdx, "Escape");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_0);
    lua_setfield(L, tableIdx, "N0");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_1);
    lua_setfield(L, tableIdx, "N1");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_2);
    lua_setfield(L, tableIdx, "N2");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_3);
    lua_setfield(L, tableIdx, "N3");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_4);
    lua_setfield(L, tableIdx, "N4");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_5);
    lua_setfield(L, tableIdx, "N5");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_6);
    lua_setfield(L, tableIdx, "N6");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_7);
    lua_setfield(L, tableIdx, "N7");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_8);
    lua_setfield(L, tableIdx, "N8");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_9);
...
#else
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_A);
    lua_setfield(L, tableIdx, "A");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_B);
    lua_setfield(L, tableIdx, "B");
#endif
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_C);
    lua_setfield(L, tableIdx, "C");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_D);
    lua_setfield(L, tableIdx, "D");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_E);
    lua_setfield(L, tableIdx, "E");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F);
    lua_setfield(L, tableIdx, "F");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_G);
    lua_setfield(L, tableIdx, "G");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_H);
    lua_setfield(L, tableIdx, "H");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_I);
    lua_setfield(L, tableIdx, "I");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_J);
    lua_setfield(L, tableIdx, "J");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_K);
    lua_setfield(L, tableIdx, "K");
#if PLATFORM_3DS
    lua_pushinteger(L, 0);
    lua_setfield(L, tableIdx, "L");
#else
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_L);
    lua_setfield(L, tableIdx, "L");
#endif
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_M);
    lua_setfield(L, tableIdx, "M");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_N);
    lua_setfield(L, tableIdx, "N");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_O);
    lua_setfield(L, tableIdx, "O");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_P);
    lua_setfield(L, tableIdx, "P");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_Q);
    lua_setfield(L, tableIdx, "Q");
#if PLATFORM_3DS
    lua_pushinteger(L, 0);
    lua_setfield(L, tableIdx, "R");
#else
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_R);
    lua_setfield(L, tableIdx, "R");
#endif
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_S);
    lua_setfield(L, tableIdx, "S");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_T);
    lua_setfield(L, tableIdx, "T");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_U);
    lua_setfield(L, tableIdx, "U");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_V);
    lua_setfield(L, tableIdx, "V");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_W);
...
#else
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_X);
    lua_setfield(L, tableIdx, "X");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_Y);
    lua_setfield(L, tableIdx, "Y");
#endif
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_Z);
    lua_setfield(L, tableIdx, "Z");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_SPACE);
    lua_setfield(L, tableIdx, "Space");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_ENTER);
    lua_setfield(L, tableIdx, "Enter");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_BACKSPACE);
    lua_setfield(L, tableIdx, "Backspace");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_TAB);
    lua_setfield(L, tableIdx, "Tab");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_SHIFT_L);
    lua_setfield(L, tableIdx, "ShiftL");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_CONTROL_L);
    lua_setfield(L, tableIdx, "ControlL");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_ALT_L);
    lua_setfield(L, tableIdx, "AltL");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_SHIFT_R);
    lua_setfield(L, tableIdx, "ShiftR");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_CONTROL_R);
    lua_setfield(L, tableIdx, "ControlR");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_ALT_R);
    lua_setfield(L, tableIdx, "AltR");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_INSERT);
    lua_setfield(L, tableIdx, "Insert");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_DELETE);
    lua_setfield(L, tableIdx, "Delete");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_HOME);
    lua_setfield(L, tableIdx, "Home");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_END);
    lua_setfield(L, tableIdx, "End");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_PAGE_UP);
    lua_setfield(L, tableIdx, "PageUp");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_PAGE_DOWN);
...
#else
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_UP);
    lua_setfield(L, tableIdx, "Up");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_DOWN);
    lua_setfield(L, tableIdx, "Down");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_LEFT);
    lua_setfield(L, tableIdx, "Left");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_RIGHT);
    lua_setfield(L, tableIdx, "Right");
#endif

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD0);
    lua_setfield(L, tableIdx, "Numpad0");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD1);
    lua_setfield(L, tableIdx, "Numpad1");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD2);
    lua_setfield(L, tableIdx, "Numpad2");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD3);
    lua_setfield(L, tableIdx, "Numpad3");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD4);
    lua_setfield(L, tableIdx, "Numpad4");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD5);
    lua_setfield(L, tableIdx, "Numpad5");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD6);
    lua_setfield(L, tableIdx, "Numpad6");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD7);
    lua_setfield(L, tableIdx, "Numpad7");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD8);
    lua_setfield(L, tableIdx, "Numpad8");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_NUMPAD9);
    lua_setfield(L, tableIdx, "Numpad9");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F1);
    lua_setfield(L, tableIdx, "F1");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F2);
    lua_setfield(L, tableIdx, "F2");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F3);
    lua_setfield(L, tableIdx, "F3");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F4);
    lua_setfield(L, tableIdx, "F4");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F5);
    lua_setfield(L, tableIdx, "F5");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F6);
    lua_setfield(L, tableIdx, "F6");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F7);
    lua_setfield(L, tableIdx, "F7");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F8);
    lua_setfield(L, tableIdx, "F8");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F9);
    lua_setfield(L, tableIdx, "F9");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F10);
    lua_setfield(L, tableIdx, "F10");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F11);
    lua_setfield(L, tableIdx, "F11");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_F12);
    lua_setfield(L, tableIdx, "F12");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_PERIOD);
    lua_setfield(L, tableIdx, "Period");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_COMMA);
    lua_setfield(L, tableIdx, "Comma");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_PLUS);
    lua_setfield(L, tableIdx, "Plus");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_MINUS);
    lua_setfield(L, tableIdx, "Minus");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_COLON);
    lua_setfield(L, tableIdx, "Colon");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_QUESTION);
    lua_setfield(L, tableIdx, "Question");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_SQUIGGLE);
    lua_setfield(L, tableIdx, "Squiggle");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_LEFT_BRACKET);
    lua_setfield(L, tableIdx, "LeftBracket");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_BACK_SLASH);
    lua_setfield(L, tableIdx, "BackSlash");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_RIGHT_BRACKET);
    lua_setfield(L, tableIdx, "RightBracket");
    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_QUOTE);
    lua_setfield(L, tableIdx, "Quote");

    lua_pushinteger(L, KeyCode::POLYPHASE_KEY_DECIMAL);
