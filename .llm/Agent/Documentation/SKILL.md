...
| `.llm/ControlServer.md` | REST API server for remote editor control, Crow HTTP, addon routes |
...
### New Controller Server REST Endpoint

1. Add route in `Engine/Source/Editor/ControllerServer/ControllerServerRoutes.cpp`.
2. Use `CROW_ROUTE(app, "/api/your/path").methods("GET"_method)([server](const crow::request& req) { ... });`
3. Wrap all engine API calls in `server->QueueCommand(lambda)` for thread safety.
4. Use `crow::json::wvalue` for building JSON responses, `crow::json::load(body)` for parsing.
5. Call `LogRequest(server, "METHOD", "/api/your/path")` for request logging.
6. Return `crow::response(200, "application/json", result)`.
7. See `.llm/ControlServer.md` for architecture details.

### New Controller Server Addon Route (Native Addon)

1. Add callback typedef if needed in `Engine/Source/Plugins/EditorUIHooks.h`.
2. Call `api->editorUI->RegisterControllerRoute(hookId, method, path, callback, userData)` in addon OnLoad.
3. Implement `ControllerRouteCallback` — write JSON into `responseBuffer`.
4. Routes are matched via `CROW_CATCHALL_ROUTE` in ControllerServerRoutes.cpp.
